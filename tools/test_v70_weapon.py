#!/usr/bin/env python3
"""Deterministic V7.0 weapon evolution and signature-slot regression gate."""
...
JS = ROOT / "tools" / "test_v70_weapon_browser.js"
...
def main() -> int:
    ...
    session = f"v70weapon_{server.pid}"
    base = ["npx.cmd", "--yes", "--package", "@playwright/cli", "playwright-cli", "--session", session]
    url = f"http://127.0.0.1:{port}/index.html?test=weapon"
    script_path = None
    try:
        time.sleep(.25)
