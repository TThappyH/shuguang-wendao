#!/usr/bin/env python3
"""Deterministic V8 six-path resonance composition and UI gate."""
...
JS = ROOT / "tools" / "test_v8_resonance_paths_browser.js"
...
def decode(stdout: str) -> dict:
    for line in reversed([x.strip() for x in stdout.splitlines() if x.strip()]):
        ...
        if isinstance(value, dict) and "passed" in value:
            return value
    raise RuntimeError(f"Playwright CLI did not return path JSON: {stdout[-2000:]}")
...
def main() -> int:
    ...
    session = f"v8paths_{server.pid}"
    base = ["npx.cmd", "--yes", "--package", "@playwright/cli", "playwright-cli", "--session", session]
    url = f"http://127.0.0.1:{port}/index.html?test=v8-paths"
    script_path = None
    try:
        time.sleep(.25)
