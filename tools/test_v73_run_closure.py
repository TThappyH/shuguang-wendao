#!/usr/bin/env python3
"""Deterministic V7.3 complete-run record and settlement regression gate."""
...
JS = ROOT / "tools" / "test_v73_run_closure_browser.js"
...
def main() -> int:
    ...
    session = f"v73closure_{server.pid}"
    base = ["npx.cmd", "--yes", "--package", "@playwright/cli", "playwright-cli", "--session", session]
    url = f"http://127.0.0.1:{port}/index.html?test=run-closure"
    script_path = None
    try:
        time.sleep(.25)
