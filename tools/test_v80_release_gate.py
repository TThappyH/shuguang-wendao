#!/usr/bin/env python3
"""V8 release gate: cold HTTP launch plus a deterministic complete single-run path."""
...
JS = ROOT / "tools" / "test_v80_release_gate_browser.js"
...
def main() -> int:
    ...
    session = f"v80gate_{server.pid}"
    base = ["npx.cmd", "--yes", "--package", "@playwright/cli", "playwright-cli", "--session", session]
    url = f"http://127.0.0.1:{port}/index.html?test=v8-gate"
    script_path = None
    try:
        time.sleep(.25)
