#!/usr/bin/env python3
"""Deterministic V7.1 Qingyao external-model motion-state regression gate."""
...
JS = ROOT / "tools" / "test_v71_motion_browser.js"
...
def main() -> int:
    ...
    session = f"v71motion_{server.pid}"
    base = ["npx.cmd", "--yes", "--package", "@playwright/cli", "playwright-cli", "--session", session]
    url = f"http://127.0.0.1:{port}/index.html?test=motion"
    script_path = None
    try:
        time.sleep(.25)
